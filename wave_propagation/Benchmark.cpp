...
void Benchmark::run_scaling(Network& net, int steps, ScheduleType st, int chunk,
    ...
    double T1=0.0, T1s=0.0;
    {
        ...
        for (int r=0;r<reps;++r) {
            ...
            wp.run(steps, st, chunk, sm, "results/energy_trace.dat");
    ...
    for (int p : threads_list) {
        ...
        for (int r=0;r<reps;++r) {
            ...
            net.setAll(0.0); net.setInitialImpulseCenter(1.0);
            double t0 = omp_get_wtime();
            wpp.run(steps, st, chunk, sm, "results/energy_trace.dat");
            double t1 = omp_get_wtime();
            tp.push_back(t1 - t0);
        }
        double mp = mean(tp);
        double sp = stdev(tp, mp);
    ...
    for (auto st : sts){
        for (int c : chunks){
            ...
            for (int r=0;r<reps;++r){
                ...
                net.setAll(0.0); net.setInitialImpulseCenter(1.0);
                double t0 = omp_get_wtime();
                wp.run(steps, st, c, sm, "results/energy_trace.dat");
                double t1 = omp_get_wtime();
                tt.push_back(t1 - t0);
            }
...
void Benchmark::run_tasks_vs_for(Network& net, int steps, ScheduleType st, int chunk,
    ...
    // tasks
    {
        ...
        for (int r=0;r<reps;++r){
            ...
            net.setAll(0.0); net.setInitialImpulseCenter(1.0);
            double t0 = omp_get_wtime();
            wp.process_with_tasks(steps, grain, sm, "results/energy_trace.dat");
            double t1 = omp_get_wtime();
            tt.push_back(t1 - t0);
        }
        double mT = mean(tt), sT = stdev(tt, mT);
