...
// Construye un grafo tipo Erdős–Rényi aproximado mediante "mean_degree" por nodo.
...
// Watts-Strogatz: comienza con un anillo con 2k vecinos y re-alambra cada arista con prob beta
void Network::makeSmallWorld(int k, double beta) {
    ...
    for (int i=0;i<N_; ++i){
        for (int d=1; d<=k; ++d){
            int j1 = (i+d) % N_;
            int j2 = (i-d+N_) % N_;
            nodes_[i].addNeighbor(j1);
            nodes_[j1].addNeighbor(i);
            nodes_[i].addNeighbor(j2);
            nodes_[j2].addNeighbor(i);
        }
    }
    // re-alambrado
    ...
    for (int i=0;i<N_; ++i){
        for (int d=1; d<=k; ++d){
            int j = (i+d) % N_;
            if (U(rng) < beta){
                // quitar enlace i-j "lógicamente": no removemos del vector para simplicidad, evitamos duplicación agregando otro vecino aleatorio
                int newj = unif(rng);
                if (newj==i) continue;
...
void Network::setInitialImpulseCenter(double A0) {
    ...
    if (is2D_) {
        int cx = Lx_/2, cy = Ly_/2;
        int ic = cy*Lx_ + cx;
        nodes_[ic].setPrev(A0);
        nodes_[ic].setAmplitude(A0);
    } else {
        int c = N_/2;
        nodes_[c].setPrev(A0); 
        nodes_[c].setAmplitude(A0);
    }
}
