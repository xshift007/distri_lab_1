...
def plot_schedule_chunk(path='results/schedule_vs_chunk.dat'):
    ...
    for sched in schedules:
        mask = d[:,0]==sched
        ch = d[mask][:,1].astype(int)
        mT = d[mask][:,2].astype(float)
        sT = d[mask][:,3].astype(float)
        idx = np.argsort(ch)
        plt.figure()
        plt.errorbar(ch[idx], mT[idx], yerr=sT[idx], marker='o')
        plt.xlabel('Chunk size'); plt.ylabel('Tiempo [s]')
        plt.title(f'Tiempo vs Chunk ({sched})'); plt.grid(True)
...
def plot_sync_methods(path='results/sync_methods.dat'):
    ...
    plt.figure(); plt.bar(x, mT, yerr=sT); plt.xticks(x, methods)
...
def plot_tasks_vs_for(path='results/tasks_vs_for.dat'):
    ...
    plt.figure(); plt.bar(x, mT, yerr=sT); plt.xticks(x, modes)
...
def plot_tasks_grain(path='results/tasks_grain.dat'):
    d = safe_load(path)
    if d is None: return
    grains = d[:,0].astype(int); mT = d[:,1].astype(float); sT = d[:,2].astype(float)
    idx = np.argsort(grains)
    plt.figure(); plt.errorbar(grains[idx], mT[idx], yerr=sT[idx], marker='o')
    plt.xlabel('Grain (tamaño de bloque de tarea)'); plt.ylabel('Tiempo [s]')
    plt.title('Tasks: barrido de granularidad'); plt.grid(True)
    plt.savefig('results/tasks_grain.png', dpi=200, bbox_inches='tight')
...
def main():
    ...
    plot_tasks_vs_for()
    plot_tasks_grain()
    print("Listo: gráficos en results/*.png")
