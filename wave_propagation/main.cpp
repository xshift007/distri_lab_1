...
static void usage(){
    std::cout << "Uso: ./wave_propagation [opciones]\n"
              ...
              << "  --S0 0.1 --omega 0.5\n"
              << "  --schedule {static,dynamic,guided} --chunk 1\n"
...
static Args parse_args(int argc, char** argv){
    ...
    for (int i=1;i<argc;++i){
        ...
        if (k=="--network"){ need(i); a.network=argv[++i]; }
            ...
            if (k=="--benchmark") a.do_bench = true;
...
int main(int argc, char** argv){
    ...
    if (args.network == "1d") {
        net.makeRegular1D(false);
    } else if (args.network == "2d") {
        net.makeRegular2D(args.Lx, args.Ly, false);
    } else if (args.network == "random") {
        net.makeRegular1D(false);
        net.makeRandom(10.0); // grado medio aproximado
    } else if (args.network == "smallworld") {
        net.makeSmallWorld(2, 0.05);
    } else {
        std::cerr << "Tipo de red no reconocido, usando 2d por defecto.\n";
        net.makeRegular2D(args.Lx, args.Ly, false);
    }
    net.setAll(0.0);
    ...
    if (args.do_bench) {
        ...
        std::cout << "Benchmarks listos. Revisa carpeta results/ y usa scripts/plot_speedup.py\n";
        return 0;
    }
    ...
    // Demostración de cláusulas de datos (lastprivate/firstprivate/private/shared)
