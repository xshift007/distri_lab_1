...
void WavePropagator::step_schedule(ScheduleType st, int chunk) {
    ...
    // Cálculo de fuente S(t) en región paralela, ejemplo de 'single'
    double s_val = 0.0;
    #pragma omp parallel
    {
        #pragma omp single
        ...
        if (net_->is2D()) {
            ...
            // 1D/random/smallworld: simple parallel for
            if (st == ScheduleType::Static) {
        ...
        // muestra de 'nowait' arriba; insertamos barrera explícita aquí antes de usar resultados
        #pragma omp barrier
    }
    tcur_ += dt_;
}
...
// Demostración de private/shared/firstprivate/lastprivate
void WavePropagator::demo_data_clauses(const char* out_path){
    ...
    #pragma omp parallel for default(none) shared(nodes,N,s_const) private(captured_last) firstprivate(s_const) lastprivate(captured_last)
    for (int i=0;i<N;++i){
        double a = nodes[i].getPrev() + s_const;
        captured_last = a; // la última iteración asigna fuera por 'lastprivate'
    }
